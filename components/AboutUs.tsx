...
export default function AboutUs() {
  ...
  // Aggressive parallax for the text section
  const yText = useTransform(scrollYProgress, [0, 1], [350, -350]);
  ...
  return (
    <section id="about" className="relative w-full py-24 mb-12 bg-background text-white select-none overflow-hidden" ref={containerRef}>
      ...
      <div className="max-w-[85vw] mx-auto flex flex-col gap-16 md:gap-32 relative z-10">
        ...
        <div className="flex flex-col items-center justify-center text-center space-y-6">
            <motion.div
                ...
                className="px-6 py-2 rounded-full border border-white/20 bg-white/5 backdrop-blur-md text-sm uppercase tracking-widest font-medium shadow-[0_4px_24px_rgba(255,255,255,0.05)]"
            >
                About Us
            </motion.div>
            <motion.h2 
        ...
        >
          ...
          <motion.div variants={itemVariants} className="md:col-span-1 md:row-span-1 flex flex-col p-8 rounded-[2rem] bg-[#141414] border border-white/5 relative overflow-hidden group hover:-translate-y-1 transition-all duration-500 hover:border-white/20 hover:shadow-[0_8px_32px_rgba(255,255,255,0.05)] justify-between">
            <div>
              <h4 className="text-5xl lg:text-6xl font-display font-semibold mb-1">50+</h4>
          ...
          {/* Card 3: Team (Purple Theme like reference) */}
          ...
          <motion.div variants={itemVariants} className="md:col-span-1 md:row-span-1 flex flex-col p-8 rounded-[2rem] bg-gradient-to-tr from-[#111] to-[#1c1c1c] border border-white/5 relative overflow-hidden group hover:-translate-y-1 transition-all duration-500 hover:border-accent/40 justify-between">
            <div>
              <h4 className="text-5xl lg:text-6xl font-display font-semibold mb-1">20+</h4>
          ...
          <motion.div variants={itemVariants} className="md:col-span-1 md:row-span-1 flex flex-col p-8 rounded-[2rem] bg-[#0A0A0A] border border-white/10 relative overflow-hidden group hover:-translate-y-1 transition-all duration-500 hover:border-accent justify-between shadow-lg">
            ...
            <div className="relative z-10">
              <h4 className="text-5xl lg:text-6xl font-display font-semibold mb-1 text-accent drop-shadow-md">10M+</h4>
        ...
        {/* Separator / Mission Statement */}
        <div className="flex flex-col w-full" ref={scrollTargetRef}>
            <div className="text-center max-w-5xl mx-auto mb-32 md:mb-56 -mt-8 md:-mt-16 relative z-20">
                <motion.div 
                    style={{ y: yText }}
                ...
                >
                    ...
                    <p className="text-white/60 text-lg md:text-2xl leading-relaxed max-w-4xl mx-auto font-light">
                        At Monarch, we don’t believe in posting for the sake of posting. We focus on <span className="text-white font-medium">content that educates, sparks conversation, and builds real brand presence.</span>
                    </p>
            ...
            {/* Founder profiles */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 w-full max-w-[60rem] mx-auto mt-4 px-4 md:px-0 overflow-visible">
...
function FounderCard({ name, title, img1, img2 }: { name: string, title: string, img1: string, img2: string }) {
    // Removed individual useScroll and React state (isHovered) to prevent CPU thrashing
    // Hover effects are now purely CSS driven via the `group` class.
    ...
    return (
        <div className="w-full relative flex flex-col items-center group cursor-pointer">
            ...
            <div className="relative w-[80%] max-w-[320px] aspect-[4/5] rounded-[2.5rem] overflow-hidden bg-white/5 border border-white/10 mb-8 shadow-2xl transform-gpu">
                {/* Fallback pattern if image is slow */}
                ...
                {/* Image 1 (Fades out and scales up on parent hover) */}
                <Image 
                    src={img1} 
                    alt={name}
                    fill
                    ...
                    priority
                />
                {/* Image 2 (Revealed on parent hover) */}
                <Image 
                    src={img2} 
                    alt={name}
                    fill
