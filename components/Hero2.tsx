...
export default function Hero2() {
    ...
    // NEW: We introduce a strict lock state that defaults to false.
    ...
    useEffect(() => {
        if (earlyReveal) {
            ...
            const phaseTwo = setTimeout(() => {
                ...
                // NEW: Unlock the mouse parallax ONLY after the duration of phaseTwo and contentTimer 
                // have completely finished (1.5s animation duration + buffer).
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        // STRICT LOCK: We completely ignore the mouse until animations are done.
        // Because the variables stay at 0, no math or GPU rendering is triggered.
        if (!parallaxUnlocked) return;
            ...
            <div className="absolute inset-0 w-full h-full" style={{ transformStyle: "preserve-3d" }}>
                ...
                >
                    <img
                        ...
                        draggable={false}
                        className="absolute bottom-0 left-0"
                        style={{ width: "22vw", height: "auto", maxWidth: "none" }}
                    />
                </motion.div>
                ...
                >
                    ...
                    >
                        ...
                        <p className={`subtitle-reveal ${earlyReveal ? "playing" : ""} text-sm md:text-[1.1vw] text-[#EFEBDF] max-w-3xl pointer-events-auto leading-relaxed mb-1`}>
                            Attention is the highest currency, we are helping you to mine it
                        </p>
                    </motion.div>
                </motion.div>
                ...
                <motion.div
                    ...
                    className="absolute bottom-[8vh] left-0 w-full flex flex-col items-center justify-center z-[100] pointer-events-auto"
                >
                    <motion.button
                        onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "instant" })}
                        onPointerMove={(e) => {
                            // STRICT LOCK applied to the button hover physics as well
                            if (!parallaxUnlocked) return;
                            const rect = e.currentTarget.getBoundingClientRect();
                            buttonX.set((e.clientX - (rect.left + rect.width / 2)) * 0.15);
                            buttonY.set((e.clientY - (rect.top + rect.height / 2)) * 0.15);
                        ...
                        style={{ x: buttonSpringX, y: buttonSpringY }}
                        className="group relative overflow-hidden rounded-full py-4 px-10"
                    >
                        <div className="absolute inset-0 bg-accent rounded-full -z-10" />
                        <div className="absolute inset-0 bg-[#27701B] translate-y-[100%] group-hover:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] rounded-full z-0" />
                        <div className="relative z-10 flex items-center justify-center text-base md:text-[1.1vw] font-medium text-black group-hover:text-primary transition-colors duration-300">
