...
export default function PhysicsThrow() {
  ...
  useEffect(() => {
    ...
    const blocks = blocksData.map((b) =>
      Bodies.rectangle(width * b.x, height - b.h / 2 - 20, 56, b.h, {
        ...
        friction: 0.5,
        restitution: 0.2,
        render: { fillStyle: "#1a1a1a", strokeStyle: "#333", lineWidth: 1 },
      })
    );
  ...
  return (
    <div className="w-full bg-black py-24 flex justify-center">
      {/* aspect-[2/1] ensures the div has height before canvas loads 
        max-h-[80vh] prevents it from becoming too tall on huge screens
      */} 
      <div
        ref={sceneRef}
        className="relative w-full max-w-[1200px] aspect-[2/1] max-h-[80vh] cursor-grab active:cursor-grabbing"
